...
each_rep <- function(replication){
  ...
  state <- cv.glmnet(X_train,y_train,alpha = logistic_star_AUROC,family = "binomial",type.measure = "auc")
  ...
  return(result)
  #save("result", file = sprintf("%s/%s/logistic_regression_%s_measure_AUROC_replication_%d_result.RData", result_path, cohort, pathway, replication))
}
...
for (cohort in cohorts) {
  ...
  result <- foreach (replication = 1:100, .options.azure = opt, .combine='c', .errorhandling = 'pass',.packages = c('glmnet','AUC'))  %dopar% {
  ...
  for(replication in 1:100){
    result <- result1[i]
    save("result", file = sprintf("%s/%s/logistic_%s_measure_AUROC_replication_%d_result.RData", result_path, cohort, pathway, replication))
